...
/**
 * ExchangeOK i18n 补充模块翻译key脚本
 * 从src_last版本补充缺失的翻译key到模块文件中
 */
...
function supplementModuleKeys() {
  console.log('🔧 补充模块翻译key...\n');
  ...
  // 补充每个模块的缺失key
  for (const [moduleName, keys] of Object.entries(missingKeysByModule)) {
    ...
    for (const locale of LOCALES) {
      ...
      for (const keyPath of keys) {
        ...
        if (keyParts.length === 1) {
          // 一级key: eod_status, no_business_stats
...
if (require.main === module) {
  try {
    supplementModuleKeys();
  } catch (error) {
    console.error('❌ 补充失败:', error.message);
  }
}

module.exports = { supplementModuleKeys }; 