<template>
  <div class="step-content">
                ...
                <tr v-for="result in verificationData.verification_results" :key="result.currency_id">
                  <td>
                    <div class="d-flex align-items-center">
                      <currency-flag :code="result.currency_code || 'USD'" :custom-filename="result.custom_flag_filename" class="me-2" />
                      <div>
                        <strong>{{ result.currency_code }}</strong>
                        <span class="text-muted small ms-2">{{ getCurrencyDisplayName(result) }}</span>
                      </div>
                    </div>
                  </td>
          ...
          <div v-if="!allMatched" class="card mt-4">
            <div class="card-header">
              <h6 class="mb-0">
                ...
                {{ $t('eod.step4.difference_handling_title') }}
              </h6>
            </div>
            <div class="card-body">
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  <div class="d-grid">
                    <button 
                      ...
                      @click="openDifferenceAdjustModal"
                      :disabled="loading || isVerifying"
                    >
                      <font-awesome-icon :icon="['fas', 'balance-scale']" class="me-2" />
    ...
    <!-- 差额调节窗口 -->
    <EODDifferenceAdjustModal
      v-if="eodId && verificationData"
      :eod-id="eodId"
      :verification-results="verificationData?.verification_results || []"
      @adjust-completed="handleAdjustCompleted"
      @error="handleAdjustError"
    />
  </div>
</template>
...
import EODDifferenceAdjustModal from '../EODDifferenceAdjustModal.vue'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // 解构props
    const eodId = computed(() => props.eodId)
    ...
    const verifyBalance = async () => {
      ...
      try {
        ...
        const result = await eodAPI.verifyBalance(eodId.value)
    ...
    const handleDifferenceAction = async (action) => {
      ...
      try {
        ...
        const result = await eodAPI.handleBalanceDifference(eodId.value, action)
        ...
        if (result.success) {
          ...
          emit('error', result.message || '差额处理失败')
        }
      } catch (error) {
        console.error('差额处理失败:', error)
        emit('error', error.response?.data?.message || error.message || '差额处理失败')
      } finally {
        isVerifying.value = false
      }
    }

    const openDifferenceAdjustModal = () => {
      // 打开差额调节窗口
    ...
    const handleAdjustCompleted = (result) => {
      // 差额调节完成后的处理
      if (result.success) {
        // 更新验证数据
        verificationData.value = {
          verification_results: result.verification_results || [],
          all_match: result.all_match || false
        }
        ...
        if (result.all_match) {
          ...
          // 调节后仍有差额，保持在当前界面
          emit('error', t('eod.difference_adjust.adjustment_completed_with_difference'))
        }
      }
    }

    const handleAdjustError = (error) => {
      emit('error', error)
    }
    ...
    // 获取币种显示名称，支持自定义币种
    const getCurrencyDisplayName = (result) => {
      if (!result) return ''
      
      // 检查是否是自定义币种（有custom_flag_filename）
      if (result.custom_flag_filename) {
        // console.log(`[自定义币种] ${result.currency_code} 使用数据库名称: ${result.currency_name}`)
        return result.currency_name || result.currency_code
      }
      
      // 使用翻译
      return getCurrencyName(result.currency_code)
    }
    ...
    return {
      ...
      openDifferenceAdjustModal,
      handleAdjustCompleted,
      handleAdjustError,
      getDifferenceClass,
      formatAmount,
      formatDifference,
