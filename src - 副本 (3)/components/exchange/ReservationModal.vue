    ...
    const submitReservationAndViewPDF = async (formDataFromEvent) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const primaryScreenWidth = 1620  // 可配置的主屏幕宽度
          ...
          if (screenWidth > primaryScreenWidth) {
            ...
            console.log(`[ReservationModal] 🖥️ 检测到扩展显示器！窗口将打开在副屏 left=${windowLeft}px`)
          } else {
            console.log(`[ReservationModal] 📺 窗口将打开在主屏 left=${windowLeft}px`)
          }
