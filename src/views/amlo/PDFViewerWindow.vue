...
import { pdfFieldToDbField, dbFieldToPdfField, pdfDataToDbData } from '@/utils/amloFieldMapping'
  ...
  setup() {
    ...
    const submitModifications = async () => {
      ...
      try {
        ...
        console.log(`[PDFViewerWindow] Final form data prepared: ${Object.keys(finalFormData).length} fields`)
    ...
    // PDF iframe load handler
    const onPdfIframeLoad = () => {
      console.log('[PDFViewerWindow] PDF iframe loaded')

      if (!isEditMode.value) {
        console.log('[PDFViewerWindow] Not in edit mode, skipping PDF sync setup')
        return
      }

      // Setup PDF form field synchronization in edit mode
      setupPdfFormSync()
    }

    // Setup PDF form field synchronization
    const setupPdfFormSync = () => {
      console.log('[PDFViewerWindow] Setting up PDF form synchronization...')

      if (!pdfIframeRef.value) {
        console.warn('[PDFViewerWindow] PDF iframe ref not available')
        return
      }
      ...
      try {
        ...
        if (!iframeDoc) {
          console.warn('[PDFViewerWindow] Cannot access iframe document (possible CORS issue)')
          return
        }

        console.log('[PDFViewerWindow] Iframe document accessible, setting up listeners...')

        // Monitor PDF form field changes
        // Note: PDF forms in iframe may use different rendering methods
        // This is a basic implementation that works with native HTML form fields
        watchPdfFormChanges(iframeDoc)

        console.log('[PDFViewerWindow] ✅ PDF form sync setup complete')
      } catch (error) {
        console.error('[PDFViewerWindow] Error setting up PDF sync:', error)
      }
    }

    // Watch for PDF form field changes
    const watchPdfFormChanges = (iframeDoc) => {
      // Add change event listener to iframe document
      iframeDoc.addEventListener('change', (event) => {
        ...
        console.log(`[PDFViewerWindow] PDF field changed: ${target.name} = ${target.value}`)

        // Sync to right panel
        syncPdfFieldToPanel(target.name, target.value, target.type)
      })
      ...
      iframeDoc.addEventListener('input', (event) => {
        ...
        console.log(`[PDFViewerWindow] PDF field input: ${target.name} = ${target.value}`)

        // Sync to right panel (real-time)
        syncPdfFieldToPanel(target.name, target.value, target.type)
      })

      console.log('[PDFViewerWindow] PDF form change listeners added')
    }

    // Sync PDF field value to right panel
    const syncPdfFieldToPanel = (pdfFieldName, value, fieldType) => {
      // TRANSLATE: PDF field name → Database field name
      const dbFieldName = pdfFieldToDbField(pdfFieldName) || pdfFieldName

      console.log(`[PDFViewerWindow] Syncing PDF → Panel: ${pdfFieldName} (DB: ${dbFieldName}) = ${value}`)

      // Determine where to update the value
      // Check if it's a structured field or form_data field
      const structuredFields = ['customer_name', 'customer_id', 'local_amount', 'amount', 'report_no']
      ...
      if (structuredFields.includes(dbFieldName)) {
        ...
        if (fieldType === 'checkbox') {
          processedValue = value === 'on' || value === true || value === 'true'
        }
      ...
      console.log(`[PDFViewerWindow] ✅ Panel updated: ${dbFieldName}`)
    }

    // Sync panel field value to PDF
    const syncPanelFieldToPdf = (dbFieldName, value) => {
      if (!pdfSyncEnabled.value || !isEditMode.value) return
      if (!pdfIframeRef.value) return

      // TRANSLATE: Database field name → PDF field name
      const pdfFieldName = dbFieldToPdfField(dbFieldName) || dbFieldName

      console.log(`[PDFViewerWindow] Syncing Panel → PDF: ${dbFieldName} (PDF: ${pdfFieldName}) = ${value}`)
      ...
      try {
        ...
        if (!iframeDoc) {
          console.warn('[PDFViewerWindow] Cannot access iframe document')
          return
        }

        // Find the field in PDF using the translated PDF field name
        const pdfField = iframeDoc.querySelector(`[name="${pdfFieldName}"]`)

        if (pdfField) {
          if (pdfField.type === 'checkbox') {
            pdfField.checked = Boolean(value)
          } else {
            pdfField.value = value !== null && value !== undefined ? String(value) : ''
          }
          console.log(`[PDFViewerWindow] ✅ PDF field updated: ${pdfFieldName} (from ${dbFieldName})`)
        } else {
          console.warn(`[PDFViewerWindow] PDF field not found: ${pdfFieldName} (from ${dbFieldName})`)
        }
      } catch (error) {
        console.error(`[PDFViewerWindow] Error syncing panel to PDF:`, error)
      }
    }

    // Enhanced field change handler with PDF sync
    ...
    // Extract all current field values from PDF iframe
    const extractPdfFormData = () => {
      console.log('[PDFViewerWindow] 🔍 开始提取PDF字段数据...')
      console.log('[PDFViewerWindow] - pdfIframeRef存在:', !!pdfIframeRef.value)
      console.log('[PDFViewerWindow] - 编辑模式:', isEditMode.value)

      if (!pdfIframeRef.value) {
        console.warn('[PDFViewerWindow] ❌ PDF iframe引用不存在，无法提取数据')
        return null
      }
      ...
      try {
        ...
        if (!iframeDoc) {
          console.warn('[PDFViewerWindow] ❌ 无法访问iframe文档（可能是跨域限制）')
          return null
        }

        console.log('[PDFViewerWindow] ✅ iframe文档访问成功')

        const extractedPdfData = {}
        const inputs = iframeDoc.querySelectorAll('input, textarea, select')

        console.log(`[PDFViewerWindow] 找到 ${inputs.length} 个表单元素`)

        inputs.forEach((input, index) => {
          const pdfFieldName = input.name
          if (!pdfFieldName) {
            console.log(`[PDFViewerWindow] - 元素${index}: 无name属性，跳过`)
            return
          }
          ...
          if (input.type === 'checkbox') {
            value = input.checked
            extractedPdfData[pdfFieldName] = value
            if (value) { // 只记录选中的复选框
              console.log(`[PDFViewerWindow] - ✓ [${pdfFieldName}] = ${value} (checkbox)`)
            }
          } else {
            value = input.value || ''
            extractedPdfData[pdfFieldName] = value
            if (value) { // 只记录有值的字段
              console.log(`[PDFViewerWindow] - ✓ [${pdfFieldName}] = "${value}"`)
            }
          }
        })
        ...
        // TRANSLATE: PDF data → Database data
        ...
        console.log('[PDFViewerWindow] 数据库字段列表:', Object.keys(extractedDbData).join(', '))
