  ...
  <div class="pdf-viewer-window">
    ...
    <div class="pdf-footer">
      <div class="footer-hint">
        ...
        <span v-if="viewMode === 'reservation'">需要在扩展屏给客户签名时，按 Win + Shift + →</span>
        <span v-else>{{ t('amlo.pdfViewer.viewMode') || '查看模式' }}</span>
      </div>
      <div class="footer-actions">
        <!-- 预约模式：显示签名、取消、提交预约按钮 -->
        <template v-if="viewMode === 'reservation'">
        ...
        <template v-else>
          <button type="button" class="btn btn-secondary" @click="cancelAndCloseWindow">
            <i class="fas fa-times me-2"></i>{{ t('common.close') || '关闭' }}
          </button>
        </template>
      </div>
    </div>
...
export default {
  ...
  setup () {
    ...
    const viewMode = ref('reservation')  // 'view' | 'reservation'
    ...
    const loadPDF = async (forceGenerated = false) => {
      ...
      viewMode.value = route.query.mode || 'reservation'  // 默认预约模式

      console.log('[PDFViewerWindow] View mode:', viewMode.value)
