...
class AMLOPDFFillerOverlay:
    ...
    def fill_form(self, report_type: str, data: Dict[str, Any], output_path: str, flatten: bool = False, signatures: Dict[str, str] = None) -> str:
        ...
        try:
            ...
            # 创建覆盖层PDF（包含文本和签名）
            overlay_buffer, checkbox_data = self._create_overlay_pdf(data, field_mapping, template_path, signatures)
    ...
    def draw_signatures_on_canvas(self, c: canvas.Canvas, signatures: Dict[str, str], report_type: str = None):
        ...
        signature_positions = {
            'AMLO-1-01': {
                'reporter': {'x': 400, 'y': 80, 'width': 120, 'height': 40},   # 报告人签名区
                'customer': {'x': 400, 'y': 140, 'width': 120, 'height': 40},  # 交易者签名区
                'auditor': {'x': 400, 'y': 200, 'width': 120, 'height': 40}    # 审核人签名区
            },
            'AMLO-1-02': {
                'reporter': {'x': 400, 'y': 80, 'width': 120, 'height': 40},
                'customer': {'x': 400, 'y': 140, 'width': 120, 'height': 40},
