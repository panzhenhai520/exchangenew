...
class RuleEngine:
    ...
    @staticmethod
    def get_customer_24h_stats(
        db_session: Session,
        customer_id: str
    ) -> Dict[str, Any]:
        """
        获取客户24小时内的交易统计（用于高频检测）
        ...
        Returns:
            统计结果字典:
            {
                "transaction_count_24h": 24小时内交易次数,
                "cumulative_amount_24h": 24小时内累计金额,
                "first_transaction_time": 第一笔交易时间,
                "last_transaction_time": 最后一笔交易时间
            }
        """
        try:
            ...
            result = db_session.execute(
                sql,
                {'customer_id': customer_id, 'start_time': start_time}
            )
            ...
            if row:
                return {
                    'transaction_count_24h': int(row[0] or 0),
                    'cumulative_amount_24h': float(row[1] or 0),
                    'first_transaction_time': str(row[2]) if row[2] else None,
                    'last_transaction_time': str(row[3]) if row[3] else None
                }

            return {
                'transaction_count_24h': 0,
                'cumulative_amount_24h': 0.0,
                'first_transaction_time': None,
                'last_transaction_time': None
            }

        except Exception as e:
            print(f"Error getting 24h stats for {customer_id}: {str(e)}")
            return {
                'transaction_count_24h': 0,
                'cumulative_amount_24h': 0.0,
                'first_transaction_time': None,
