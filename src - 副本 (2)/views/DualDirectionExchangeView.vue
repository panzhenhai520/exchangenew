  ...
  methods: {
    ...
    async fillTransactionFromReservation(reservation) {
      try {
        ...
        // 2. 解析form_data，填充面值组合
        if (reservation.form_data) {
          console.log('[fillTransactionFromReservation] 预约表单数据:', reservation.form_data)

          // 清空现有组合
          this.denominationCombinations = []

          // 根据direction和currency构建组合
          // 注意：这里需要根据实际的form_data结构来解析
          // 假设form_data中包含币种、金额、方向等信息

          const { direction, currency_id, amount, local_amount, rate } = reservation

          if (currency_id && amount) {
            // 添加一个组合（简化版，实际可能需要更复杂的逻辑）
