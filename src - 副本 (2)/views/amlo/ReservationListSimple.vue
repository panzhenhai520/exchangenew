...
export default {
  ...
  setup() {
    ...
    const viewPDF = async (item) => {
      ...
      try {
        // Generate PDF directly from reservation (原来的方式)
        console.log('[ReservationListSimple] Generating PDF - reservation ID:', item.id)

        // Download PDF via API (token automatically attached by axios)
        const response = await api.get(`/amlo/reservations/${item.id}/generate-pdf`, {
        ...
        if (!pdfWindow) {
          ...
          alert(t('amlo.reservation.downloadStarted') || 'PDF下载已开始')
        }
        ...
        console.error('[ReservationListSimple] Generate PDF failed:', error)
        alert(t('amlo.reservation.pdfError') || 'PDF生成失败: ' + (error.response?.data?.message || error.message))
      }
    }
