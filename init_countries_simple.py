...
Simplified version using direct SQL without ORM models
"""
...
def init_countries():
    """Initialize countries data using direct SQL"""
    try:
        ...
        if existing_count > 0:
            ...
            response = input("Clear and re-import? (yes/no): ")
        ...
        # Insert countries
        print(f"\nInserting {len(COUNTRIES_DATA)} countries...")

        insert_sql = """
            INSERT INTO countries
            (country_code, country_name_en, country_name_zh, country_name_th,
             phone_code, currency_code, is_active, sort_order)
        ...
        # Show top 5
        cursor.execute("SELECT country_code, country_name_en, country_name_zh, sort_order FROM countries ORDER BY sort_order LIMIT 5")
        top_countries = cursor.fetchall()
        print("Top 5 countries by sort_order:")
        for c in top_countries:
            print(f"  {c[0]}: {c[1]} / {c[2]} (sort: {c[3]})")
...
if __name__ == '__main__':
    print("Countries Data Initialization Script (Direct SQL)")
    print("=" * 60)
    success = init_countries()
    if success:
