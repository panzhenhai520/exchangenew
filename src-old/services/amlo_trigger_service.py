    ...
    def _prepare_transaction_data(
        ...
        if transaction.customer_id:
            try:
                ...
                # 24小时统计（用于高频检测）
                stats_24h = RuleEngine.get_customer_24h_stats(
                    session,
                    transaction.customer_id
                )
                data['transaction_count_24h'] = stats_24h.get('transaction_count_24h', 0)
                data['cumulative_amount_24h'] = float(stats_24h.get('cumulative_amount_24h', 0))
            except Exception as e:
                logger.warning(f"获取客户统计失败: {str(e)}")
                data['cumulative_amount_30d'] = 0
                data['transaction_count_30d'] = 0
                data['transaction_count_24h'] = 0
                data['cumulative_amount_24h'] = 0
