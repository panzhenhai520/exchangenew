...
const CURRENCY_TRANSLATIONS = {
  ...
  'SAR': { 'zh': '沙特里亚尔', 'en': 'Saudi Riyal', 'th': 'ริยัลซาอุดิอาระเบีย' },
  'AED': { 'zh': '阿联酋迪拉姆', 'en': 'UAE Dirham', 'th': 'เดอร์แฮมสหรัฐอาหรับเอมิเรตส์' },
  'TRY': { 'zh': '土耳其里拉', 'en': 'Turkish Lira', 'th': 'ลีราตุรกี' },
...
export function getCurrencyName(currencyCode, lang = null, apiCurrencyNames = null) {
  ...
  try {
    ...
    console.warn(`Failed to get i18n currency translation for ${currencyCode}:`, error);
  }
 ...
 */
export async function initializeCurrencyTranslations() {
  console.log('🔄 开始初始化币种翻译...');
  try {
    await loadCurrencyTranslationsFromAPI();
    console.log('✅ 币种翻译初始化完成');
  } catch (error) {
    console.warn('币种翻译初始化失败，使用本地翻译:', error);
  }
} 