...
export default {
  ...
  emits: ['submit', 'update:formData', 'view-pdf', 'fill-report'],
  setup(props, { emit }) {
    ...
    // 填写报告 - 验证表单后触发提交和查看PDF事件
    const printBlankPDF = async () => {
      console.log('[DynamicFormImproved] ===== 填写报告按钮被点击 =====')
      console.log('[DynamicFormImproved] reportType:', props.reportType)
      console.log('[DynamicFormImproved] formData:', JSON.stringify(formData.value, null, 2))
      ...
      try {
        ...
        if (hasError) {
          fieldErrors.value = errors
          alert(t('amlo.form.validationFailed') || '表单验证失败，请检查必填字段')
          return
        }
        ...
        // 2. 触发填写报告事件，让父组件处理提交预约和打开PDF
        console.log('[DynamicFormImproved] 表单验证通过，触发fill-report事件')
        emit('fill-report', formData.value)

      } catch (error) {
        console.error('[DynamicFormImproved] ❌ 表单验证失败:', error)
        alert(`${t('amlo.form.validationFailed') || '表单验证失败'}: ${error.message}`)
      }
    }
