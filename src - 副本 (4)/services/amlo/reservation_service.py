...
class ReservationService:
    ...
    def create_reservation(
        ...
        Args:
            ...
            local_amount: Amount in local currency (THB)
            rate: Exchange rate
            branch_id: Branch ID
        ...
        try:
            ...
            if not report_type:
                return False, None, "Report type is required"
            if not branch_id:
                return False, None, "Branch ID is required"
            if not user_id:
                return False, None, "User ID is required"

            # Prepare form data JSON
            form_data_json = json.dumps(form_data, ensure_ascii=False) if form_data else '{}'

            # Generate reservation number (using timestamp-based format)
            reservation_no = f"RES-{branch_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}"

            # Insert reservation (without currency_code - field doesn't exist in table)
            insert_sql = text("""
                INSERT INTO Reserved_Transaction (
                    reservation_no, report_type, customer_id, customer_name,
                    customer_country_code, direction, currency_id,
                    amount, local_amount, rate, branch_id, operator_id,
                    linked_transaction_id, form_data, status, created_at, updated_at
                ) VALUES (
                    :reservation_no, :report_type, :customer_id, :customer_name,
                    :customer_country_code, :direction, :currency_id,
                    :amount, :local_amount, :rate, :branch_id, :operator_id,
                    :linked_transaction_id, :form_data, 'pending', NOW(), NOW()
                )
            """)
            ...
            reservation_id = session.execute(get_id_sql).scalar()
            ...
            logger.info(f"[ReservationService] Created reservation {reservation_id} by user {user_id}")
            return True, reservation_data, None

        except Exception as e:
            session.rollback()
            logger.error(f"[ReservationService] Error creating reservation: {e}", exc_info=True)
            return False, None, f"Failed to create reservation: {str(e)}"
        ...
        try:
            sql = text("""
                SELECT
                    ...
                    denomination_data, created_at, updated_at
            ...
            reservation_data = {
                ...
                'denomination_data': AMLODatabaseHelper.parse_form_data(result[12]),
    ...
    def list_reservations(
        ...
        try:
            ...
            for item in items:
                ...
                # Parse denomination_data JSON if present
                if item.get('denomination_data'):
                    item['denomination_data'] = AMLODatabaseHelper.parse_form_data(item['denomination_data'])
        ...
        try:
            ...
            sql = text("""
                SELECT
                    ...
                    form_data, denomination_data, created_at, audit_time,
            ...
            reservation_data = {
                ...
                'denomination_data': AMLODatabaseHelper.parse_form_data(result[14]),
                'created_at': result[15].isoformat() if result[15] else None,
                'audit_time': result[16].isoformat() if result[16] else None,
                'operator_id': result[17],
                'auditor_id': result[18],
                'rejection_reason': result[19]
            }
    ...
    def update_reservation_form_data(
        ...
        user_id: int,
        denomination_data: Optional[Dict[str, Any]] = None
    ) -> Tuple[bool, Optional[str]]:
        """
        Update reservation form data and optionally denomination data
        ...
        Args:
            ...
            denomination_data: Optional denomination data to update
        ...
        try:
            ...
            if denomination_data is not None:
                denomination_data_json = json.dumps(denomination_data, ensure_ascii=False)
                update_sql = text("""
                ...
                session.execute(update_sql, {
                    ...
                    'form_data': form_data_json,
                    'denomination_data': denomination_data_json
                })
            else:
                update_sql = text("""
                    UPDATE Reserved_Transaction
            ...
            logger.info(f"[ReservationService] Updated reservation {reservation_id} by user {user_id}")
            return True, None
    ...
    @staticmethod
    def resubmit_reservation(
        session,
        reservation_id: int,
        form_data: Dict[str, Any],
        denomination_data: Optional[Dict[str, Any]],
        previous_report_number: Optional[str],
        remarks: Optional[str],
        user_id: int
    ) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        """
        Resubmit a rejected or completed reservation with updated data
        This creates a link to the previous report via previous_report_number

        Args:
            session: SQLAlchemy session
            reservation_id: Reservation ID to resubmit
            form_data: Updated form data
            denomination_data: Updated denomination data
            previous_report_number: Previous report number (for amendments)
            remarks: Reason for resubmission
            user_id: User performing the resubmission

        Returns:
            Tuple of (success, new_reservation_data, error_message)
        """
        try:
            ...
            # Only allow resubmit for rejected or completed reservations
            if current_status not in ['rejected', 'completed']:
                return False, None, f"Cannot resubmit reservation with status '{current_status}'"
            ...
            # Prepare data
            form_data_json = json.dumps(form_data, ensure_ascii=False)
            denomination_data_json = json.dumps(denomination_data, ensure_ascii=False) if denomination_data else None

            # Update existing reservation to set status back to pending
            update_sql = text("""
                UPDATE Reserved_Transaction
                SET status = 'pending',
                    form_data = :form_data,
            ...
            success, updated_reservation, error = ReservationService.get_reservation_by_id(
                session, reservation_id
            )
            ...
            logger.info(f"[ReservationService] Resubmitted reservation {reservation_id} by user {user_id}")
            return True, updated_reservation, None

        except Exception as e:
            session.rollback()
            logger.error(f"[ReservationService] Error resubmitting reservation {reservation_id}: {e}", exc_info=True)
            return False, None, f"Failed to resubmit reservation: {str(e)}"
