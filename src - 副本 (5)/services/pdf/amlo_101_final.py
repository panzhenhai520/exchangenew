...
class AMLO101Final:
    ...
    def generate(self, data: Dict, path: str):
        ...
        c.setDash(1, 1)
        for i in range(8):
            c.line(amount_col_x + 2*mm, yfl - i*2*mm, left_x + left_w - 2*mm, yfl - i*2*mm)
        c.setDash()
        ...
        c.setDash(1, 1)
        for i in range(8):
            c.line(amount_col_x_r + 2*mm, yfr - i*2*mm, right_x + right_w - 2*mm, yfr - i*2*mm)
        c.setDash()
