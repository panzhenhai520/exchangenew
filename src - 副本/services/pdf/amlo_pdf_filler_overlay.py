...
class AMLOPDFFillerOverlay:
    ...
    def _draw_signatures_with_pymupdf(self, doc, signatures: Dict[str, str]):
        ...
        # 获取当前报告类型的签名位置
        report_type = self.current_report_type or 'AMLO-1-01'
        positions = signature_positions.get(report_type, signature_positions['AMLO-1-01'])

        page = doc[0]  # 第一页
        page_height = page.rect.height  # A4 = 842pt
        ...
        for sig_type, sig_data in signatures.items():
            ...
            try:
                ...
                # 转换坐标：ReportLab (bottom-left origin) → PyMuPDF (top-left origin)
                # ReportLab: y=100 means 100pt from bottom
                # PyMuPDF: y=0 is top, need to convert: pymupdf_y = page_height - reportlab_y - height
                x0 = pos['x']
                y0 = page_height - pos['y'] - pos['height']
                x1 = x0 + pos['width']
                y1 = y0 + pos['height']

                # 创建矩形区域
                rect = fitz.Rect(x0, y0, x1, y1)
                print(f"[AMLOPDFFillerOverlay] Signature rect: {rect}")
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"[AMLOPDFFillerOverlay] Position: ({x0:.1f}, {y0:.1f}) Size: {pos['width']}x{pos['height']}")
                    else:
