...
def get_currency_denomination_rates(current_user, currency_id):
    ...
    try:
        ...
        # 查询今日该币种的面值汇率数据
        query = session.query(DenominationPublishDetail).join(
            RatePublishRecord, DenominationPublishDetail.publish_record_id == RatePublishRecord.id
        ).filter(
            RatePublishRecord.branch_id == current_user['branch_id'],
            RatePublishRecord.publish_date == today,
            RatePublishRecord.notes.like('%面值汇率发布%'),
            DenominationPublishDetail.currency_id == currency_id
        ).order_by(DenominationPublishDetail.denomination_value)
        ...
        for result in results:
            denomination_rates.append({
                ...
                'denomination_value': float(result.denomination_value),
                'denomination_type': result.denomination_type,
