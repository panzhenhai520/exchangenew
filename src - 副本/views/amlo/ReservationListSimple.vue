<template>
  <div class="container-fluid py-4">
    ...
    <!-- PDF查看器模态框 🆕 任务12: PDF报告查看窗口 -->
    <PDFViewerModal
      ref="pdfViewerRef"
      modal-id="reservationPDFViewer"
      :title="pdfViewerTitle"
      :report-type="pdfReportType"
      :pdf-source="pdfBlob"
      :allow-print="true"
      :allow-download="true"
      :allow-signature="true"
      :reservation-id="currentReservationId"
      @signature-submitted="handleSignatureSubmitted"
    />
  </div>
</template>
...
import PDFViewerModal from '@/components/amlo/PDFViewerModal.vue'
  ...
  setup() {
    ...
    const viewPDF = async (item) => {
      ...
      try {
        ...
        // Screen detection with detailed logging
        const screenWidth = window.screen.width
        const screenHeight = window.screen.height
        const screenAvailWidth = window.screen.availWidth
        const screenAvailHeight = window.screen.availHeight

        console.log('[ReservationListSimple] Screen Info:', {
          width: screenWidth,
          height: screenHeight,
        ...
        // 可配置的主屏幕宽度 - 如果检测不准确，用户可以修改这个值
        const primaryScreenWidth = 1620  // 根据您的主屏幕实际宽度调整此值

        // 多显示器检测逻辑
        let windowLeft = 100  // 默认：主屏幕
        let windowTop = 50

        if (screenWidth > primaryScreenWidth) {
          // 检测到扩展显示器
          windowLeft = primaryScreenWidth + 100  // 在副屏打开
          console.log(`[ReservationListSimple] 🖥️ 检测到扩展显示器！`)
          console.log(`[ReservationListSimple] 主屏幕宽度：${primaryScreenWidth}px`)
          console.log(`[ReservationListSimple] 总屏幕宽度：${screenWidth}px`)
          console.log(`[ReservationListSimple] 窗口将打开在 left=${windowLeft}px (副屏)`)
        } else {
          console.log(`[ReservationListSimple] 📺 未检测到扩展显示器或主屏幕宽度配置不正确`)
          console.log(`[ReservationListSimple] 当前屏幕宽度：${screenWidth}px <= 主屏宽度配置：${primaryScreenWidth}px`)
          console.log(`[ReservationListSimple] 窗口将打开在主屏幕 left=${windowLeft}px`)
          console.log(`[ReservationListSimple] 提示：如果您有扩展显示器，请检查primaryScreenWidth配置`)
        }

        const windowFeatures = `width=1800,height=1000,left=${windowLeft},top=${windowTop},resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,status=no`
        ...
        if (!pdfWindow) {
          ...
          console.log('[ReservationListSimple] PDF viewer window opened successfully')
    ...
    const handleSignatureSubmitted = (data) => {
      console.log('[ReservationListSimple] Signature submitted:', data)
      // 刷新预约列表以显示已签名状态
      loadReservations()
    }
    ...
    const handleViewPDF = async (data) => {
      ...
      try {
        ...
        // Screen detection with detailed logging (same as viewPDF)
        const screenWidth = window.screen.width
        const screenHeight = window.screen.height
        const screenAvailWidth = window.screen.availWidth
        const screenAvailHeight = window.screen.availHeight

        console.log('[ReservationListSimple] Screen Info (from DynamicForm):', {
          width: screenWidth,
          height: screenHeight,
          availWidth: screenAvailWidth,
          availHeight: screenAvailHeight,
        ...
        const primaryScreenWidth = 1920  // 根据您的主屏幕实际宽度调整此值

        let windowLeft = 100
        let windowTop = 50

        if (screenWidth > primaryScreenWidth) {
          windowLeft = primaryScreenWidth + 100
          console.log(`[ReservationListSimple] 🖥️ 检测到扩展显示器！(from DynamicForm)`)
          console.log(`[ReservationListSimple] 主屏幕宽度：${primaryScreenWidth}px`)
          console.log(`[ReservationListSimple] 总屏幕宽度：${screenWidth}px`)
          console.log(`[ReservationListSimple] 窗口将打开在 left=${windowLeft}px (副屏)`)
        } else {
          console.log(`[ReservationListSimple] 📺 未检测到扩展显示器 (from DynamicForm)`)
          console.log(`[ReservationListSimple] 当前屏幕宽度：${screenWidth}px <= 主屏宽度配置：${primaryScreenWidth}px`)
          console.log(`[ReservationListSimple] 窗口将打开在主屏幕 left=${windowLeft}px`)
        }
        ...
        if (!pdfWindow) {
          ...
          console.log('[ReservationListSimple] ✅ PDF viewer window opened successfully from DynamicForm')
          pdfWindow.focus()
        }
